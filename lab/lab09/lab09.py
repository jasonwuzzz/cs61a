...
def subseqs(s):
    ...
    if not s:
        ...
        item, nested_list = s[0], subseqs(s[1:])
        return insert_into_all(item, nested_list) + subseqs(s[1:])
...
def inc_subseqs(s):
    ...
    if not s:
        ...
        item, nested_list = s[0], inc_subseqs(s[1:])
        filtered = [s for s in nested_list if s and min(s) >= item] + [[]]
        return nested_list + insert_into_all(item, filtered)
...
def make_generators_generator(g):
    ...
    length = len(list(g()))
    def gen(i):
        """Sub-generator of g that only generates to index i"""
        for e in g():
            if i >= 0:
                yield e
                i -= 1
...
class Keyboard:
    ...
    def press(self, info):
        ...
        if info in self.buttons:
            botton = self.buttons[info]
            botton.times_pressed += 1
            return botton.key
        return ''
