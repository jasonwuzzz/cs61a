...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        Number(14)
        """
        func_val = self.operator.eval(env)
        arguments = [operand.eval(env) for operand in self.operands]
        return func_val.apply(arguments)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        # Create a new frame each time calling a function
        self.parent = self.parent.copy()
        # Bind parameters to arguments (update env)
        self.parent.update({para: arg for para, arg in zip(self.parameters, arguments)})
        return self.body.eval(self.parent)
