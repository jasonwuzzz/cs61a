...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        reflected_damage = self.damage + amount if self.armor - amount <= 0 else amount
        for bee in self.place.bees[:]:
            bee.reduce_armor(reflected_damage)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 6
        if self.digesting != 0:    # it is digesting
            self.digesting -= 1
        elif self.place.bees:   # there are bees in current place
...
class ScubaThrower(ThrowerAnt):
    """A ThrowerAnt that is watersafe but costy"""
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        current_place = self.place
        while current_place != beehive:
            bees = [(bee, distance) for bee in current_place.bees]
            ants = []
            if current_place.ant:
                ants += [(current_place.ant, distance)]
                if isinstance(current_place.ant, ContainerAnt) and current_place.ant.contained_ant:   
                    ants += [(current_place.ant.contained_ant, distance)]
            insects += bees + ants
            distance += 1
            current_place = current_place.entrance
        insects = dict(insects)
        insects.pop(self)   # Excluding the laser ant it self
        return insects
        # END Problem Optional 5
